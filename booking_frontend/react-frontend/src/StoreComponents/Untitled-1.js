...
/*  {sumbitPayment = async (e) => {
  ...
  render() {
    return (
      <div className="col s20 m5 test" style={{ background: "#fff0f3", margin: "10px" }}>
        <div class="container">
...
<div class="col-md-20 order-md-5 mb-2">
...
<td style={{width:"1000px",align:"right"}}>{this.state.temp.map(
          items =>
            <h6 value={this.state.total_items} 
            onChange={this.changeTotalItemsHandler} 
            class="my-0">{items.name} 
                      </h6>
...
<td >{this.state.temp.map(
          items =>
            <div><span class="text-muted">{items.price}</span></div>
...
<td value={this.state.total}  onChange={this.changeTotalHandler}><span>{this.state.total}  
                    </span></td>
  </tr>
</div><div>
          <div className="card-image hide-on-small-only">
...
<table>
  ...
  <td>{this.state.temp.map(
     ...
     <label htmlFor="price">Items</label>
              ...
              <div className="row">
                <a
                  onClick={this.saveCheckout.bind(this)}
