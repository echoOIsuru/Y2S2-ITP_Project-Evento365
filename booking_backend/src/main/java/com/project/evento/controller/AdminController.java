...
public class AdminController {
	...
	public ResponseEntity<Optional<Admin>> loginValidate(@RequestBody LoginValidationDto loginvalidationdto) {
		Optional<Admin> optAdmin = adminRepository.findByUsernameAndPassword(loginvalidationdto.getUsername(),
				loginvalidationdto.getPassword());
		if (optAdmin.isPresent()) {
			return ResponseEntity.ok(optAdmin);
		} else {
			Optional<Admin> optAdminEmpty = Optional.of(new Admin());
			return ResponseEntity.ok(optAdminEmpty);
		}
	...
	public ResponseEntity<List<Admin>> loginValidate(@PathVariable String keyword) {
		List<Admin> listAdmin= adminRepository.findByNameOrEmailOrAddressOrNicOrBirthdayOrMobileOrGenderOrAdminType(keyword);
