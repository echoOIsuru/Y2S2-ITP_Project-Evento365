...
public class RentalItemController {
	...
	// search function
		@GetMapping("/rentalitem/search/{keyword}")
		public ResponseEntity<List<RentalItem>> loginValidate(@PathVariable String keyword) {
			List<RentalItem> rentalitem= rentRepository.findByNameOrDescriptionOrRentalperdayOrTotalunitsOrAvailableunits(keyword);
			if (!rentalitem.isEmpty()) {
				return ResponseEntity.ok(rentalitem);
			} else {
//				List<RentalItem> rentalitemempty = List.of(new RentalItem());
